"""Quanvolutional-stem variant (UNOPTIMIZED / REFERENCE) — Henderson et al. 2019.

This is the original, pre-optimization Henderson quanvolutional layer, kept as a
clean reference implementation that maps directly to the paper (arXiv:1904.04767,
Sections 3.4 and 3.5). It deliberately has NONE of the speed optimizations of
`quanv_2d_henderson_2019.py`:

  * NO lookup table (LUT) precomputed at __init__
  * NO disk cache of LUTs
  * NO parallel (multiprocessing) circuit build
  * NO GPU-side index packing / gather

Every forward pass evaluates the quantum circuits directly via PennyLane. The
ONLY remaining speedup is per-batch bitstring deduplication (`np.unique`), which
returns numerically identical results to a naive per-patch loop. To obtain the
fully literal "one circuit evaluation per sliding-window position" behaviour,
see the NAIVE-MODE note inside `_eval_qnode_on_unique`.

Replaces every `nn.Conv2d` inside the stem's ConvBNReLU blocks
(MedViT.py:425-428) with a faithful Henderson quanvolutional layer. BatchNorm2d
and ReLU stay classical; only the convolution operator is swapped. The quanv
layer is the ORIGINAL non-trainable variant from the 2019 paper:
...
def _build_quanv_circuit(
    ...
    Returns:
        (qnode, gate_plan). The QNode takes a binary bitstring (one bit per
        qubit) as input — threshold encoding is applied classically before the
        QNode is called.
    """
    ...
    # without re-shuffling across repetitions. depth=1 matches the paper.
    gate_plan = []
...
class Quanv2d(nn.Module):
    """Henderson 2019 quanvolutional layer (UNOPTIMIZED reference).

    Drop-in replacement for `nn.Conv2d` inside `ConvBNReLU` — same input/output
    spatial semantics. Strictly non-trainable: there are NO nn.Parameter
    members; the random circuits are frozen at construction.

    Unlike the optimized variant, this class does NOT build a lookup table.
    Every forward pass evaluates the PennyLane circuits directly.
    """
    ...
    def _eval_qnode_on_unique(self, qnode, bitstrings_flat: np.ndarray) -> np.ndarray:
        ...
        NAIVE-MODE note: the only optimization here is the `np.unique` dedup,
        which is numerically identical to a per-patch loop. For the fully
        literal "one circuit evaluation per sliding-window position" behaviour,
        replace the unique/inverse logic with:

            scalars = np.empty(bitstrings_flat.shape[0], dtype=np.float32)
            for n in range(bitstrings_flat.shape[0]):
                scalars[n] = <decode(qnode(bitstrings_flat[n]))>
            return scalars
        """
        nq = self.n_qubits
        ...
        if self.decoding == "expval":
            # Decoded value = mean over qubits of (1 - <Z>) / 2 (probability of
            # measuring |1>), a non-negative pixel-like feature matching the
            # paper's "fraction of |1> outcomes" intuition.
            ...
            # counts mode: average popcount over n_shots samples, normalized by
            # n_qubits (measurement-statistics decoding, faithful to §3.5).
            for u in range(unique_bits.shape[0]):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # before the numpy round-trip; the quanv layer is non-trainable and
        # gradient flow stops here (the threshold itself is non-diff).
...
class QuanvConvBNReLU(nn.Module):
    """ConvBNReLU clone with the inner Conv2d swapped for the unoptimized Quanv2d.

    BatchNorm2d and ReLU are unchanged; only `.conv` becomes quantum. Kept as a
    standalone module (rather than monkey-patching ConvBNReLU) so the classical
    class stays untouched per the spec.
    """
...
class QMedViTHenderson2019Base(QMedViT):
    """QMedViT variant that replaces every ConvBNReLU in the stem (and only
    there) with the UNOPTIMIZED Henderson 2019 quanvolutional ConvBNReLU. The
    rest of the network — LTBs, ECBs, MHSA/MHCA, MLPs, BN, ReLU — is untouched.
    """
